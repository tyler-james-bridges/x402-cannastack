...
export async function runMigrations(sql: Sql) {
  ...
  // Queue columns for databases created before the worker queue existed
  await sql`ALTER TABLE crawl_runs ADD COLUMN IF NOT EXISTS claimed_at TIMESTAMPTZ`;
  await sql`ALTER TABLE crawl_runs ADD COLUMN IF NOT EXISTS attempts INTEGER NOT NULL DEFAULT 0`;
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_crawl_runs_claimable ON crawl_runs(started_at) WHERE status IN ('pending', 'running')`;
